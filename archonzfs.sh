...
zpool create -f -o ashift=12              \
                ...
                -O compression=lz4        \
                zroot "$ZFS"
# Create Encrypted and Unencrypted Datasets
print "Create Encrypted Dataset"
zfs create  -o mountpoint=none                       \
            -o keyformat=passphrase                  \
            -o keylocation=file:///etc/zfs/zroot.key zroot/enc

print "Create Unencrypted Dataset"
zfs create -o mountpoint=none zroot/pt

# Build Dataset Tree
print "Create Root Dataset"
zfs create -o mountpoint=none zroot/enc/ROOT
zfs create -o mountpoint=/ -o canmount=noauto zroot/enc/ROOT/default
print "Create Data Datasets"
zfs create -o mountpoint=none zroot/enc/data
zfs create -o mountpoint=/home zroot/enc/data/home
zfs create -o mountpoint=/root zroot/enc/data/home/root
print "Create System Datasets"
zfs create -o mountpoint=/var -o canmount=off zroot/enc/var
zfs create zroot/enc/var/log
zfs create -o mountpoint=/var/lib -o canmount=off zroot/enc/var/lib
zfs create zroot/enc/var/lib/libvirt
zfs create zroot/enc/var/lib/lxd
...
zfs load-key zroot
zfs mount zroot/enc/ROOT/default
zfs mount -a
mkdir -p /mnt/efi
mount "$EFI" /mnt/efi
...
# System Install
root_dataset=zroot/enc/ROOT/default
